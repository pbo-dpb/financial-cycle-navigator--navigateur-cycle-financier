...
const waterfallStyle = computed(() => {
  ...
  const daysSinceBeginningOfYear = Math.floor(
    (new Date(year, currentDate.getMonth(), currentDate.getDate()).getTime() -
      new Date(currentDate.getMonth() > 2 ? year : year - 1, 2, 1).getTime()) /
      86400000,
  );
  ...
  const elapsedRatio =
    daysSinceBeginningOfYear / ((isLeapYear ? 366 : 365) + extraDays);
  const elapsedFill = document.createElementNS(
    "http://www.w3.org/2000/svg",
    "rect",
  );
  ...
  const elapsedFillAnimation = document.createElementNS(
    "http://www.w3.org/2000/svg",
    "animate",
  );
  elapsedFillAnimation.setAttribute("attributeName", "width");
  elapsedFillAnimation.setAttribute(
    "values",
    `0;${endOfElapsedContent * 0.6};${endOfElapsedContent}`,
  );
  ...
  const elapsedStroke = document.createElementNS(
    "http://www.w3.org/2000/svg",
    "rect",
  );
  ...
  const elapsedStrokeAnimation = document.createElementNS(
    "http://www.w3.org/2000/svg",
    "animate",
  );
...
<template>
  <div
    class="col-span-2 grid grid-cols-13 rounded-lg pb-2 lg:rounded-none lg:pb-0"
    ref="waterfall"
