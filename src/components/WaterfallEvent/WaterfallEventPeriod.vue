<script setup>
    ...
    const getLeftPadding = () => {
        ...
        const percentageOfStartDateInMonth =
            startDate.getDate() / getDaysInMonth(startDate);
        return waterfallColWidth.value * percentageOfStartDateInMonth;
    };

    const getRightPadding = () => {
        const endDate = props.event.end;
        const percentageOfEndDateInMonth =
            endDate.getDate() / getDaysInMonth(endDate);
    ...
    const getBarWidth = () => {
        ...
        return (
            waterfallColWidth.value * (getMonthSpan() + 1) -
            getLeftPadding() -
            getRightPadding()
        );
    };
    ...
    const gridClasses = computed(() => {
        return ["lg:pt-2", getColStart(), getColSpan()];
    });
    ...
    const render = () => {
        return h(
            ...
            [
                h(CycleBar, {
                    class: [
                        props.event.end ? "w-full h-4" : "size-4",
                        "group-hover:drop-shadow-lg",
                        "group-open:group-hover:drop-shadow-none",
                        "transition-shadow",
                    ],
