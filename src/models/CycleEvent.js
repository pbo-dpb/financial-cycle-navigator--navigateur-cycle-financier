export default class CycleEvent {
    constructor(obj) {
        ...
        this.part_of_estimates_process =
            obj.details.part_of_estimates_process || false;

        this.previous_spending = obj.details.previous_spending || false;

        this.start = new Date(
            new Date().getFullYear(),
            parseInt(obj.details.start.split("-")[0]) - 1,
            parseInt(obj.details.start.split("-")[1]),
        );

        this.end = obj.details.end
            ? new Date(
                  new Date().getFullYear(),
                  parseInt(obj.details.end.split("-")[0]) - 1,
                  parseInt(obj.details.end.split("-")[1]),
              )
            : null;
