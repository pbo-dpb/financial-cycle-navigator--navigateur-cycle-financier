...
export default defineStore("datasource", {
  ...
  getters: {
    ...
    currentFy() {
      ...
      return (
        firstYear.toString().slice(2) + (firstYear + 1).toString().slice(2)
      );
    },
  },
    ...
    async fetchEvents() {
      ...
      if (this.fyevents.length === 0) {
        const response = await fetch(
          import.meta.env.DEV
            ? fyEvents
            : "https://pfcn-ecfp.pbo-dpb.ca/fyevents.json",
        );
        if (!response.ok) {
          throw new Error(`Response status: ${response.status}`);
        }
      ...
      let currentFyInfo = this.fyevents.find(
        (event) => event.year === this.currentFy,
      );
      ...
      this.events = currentFyInfo.events
        .map((fyevent) => {
          let fyEventObject = {
            // Do not include govdocs from other fiscal years
            govdocs:
              currentFyInfo.year === this.currentFy ? fyevent.govdocs : [],
          ...
          while (!fyEventObject.details) {
            const previousFyInfo =
              this.fyevents[this.fyevents.length - counter];
            fyEventObject.details = previousFyInfo.events.find(
              (e) => e.id === fyevent.id,
            )?.details;
            counter++;
          }
